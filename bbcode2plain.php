...
function bbcode2plain2($bbcode)
{
	...
	// Zuerst mal nach HTML konvertieren
	...
	$html = str_replace('<cite>'.$lang->quote.'</cite>', '', $html);

	$html = preg_replace('=(.*?)<blockquote><cite>(.*?)</cite>(.*?)=i',
				'$1<br />$2<br /><blockquote>$3', $html);

	// Und nun die Konvertierung
